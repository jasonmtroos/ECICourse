...
population <- 
  declare_population(
    N = 10000, 
    W = draw_binary(prob = .5, N = N),
    X = draw_ordered(x = rnorm(N, 1 + .5 * W, 2),
                     breaks = c(0, 1, 1.5, 3.5)))
```

`W` is a binary variable, and `X` is an ordered discrete variable. Values of `X` are higher when `W = 1` versus `W = 0`. (If it helps you to think of a concrete example of what these variables are, then think of `W` as gender and `X` as educational attainment; in the remainder  of the tutorial we will just call  them `W` and `X`.)
...
The experiment will measure an outcome variable `Y`. The potential outcomes differ between treatment `Z = 1` and control `Z = 0` by a constant amount.  Moreover, the potential outcomes are correlated with `W` and `X`.
...
The experiment will attempt to measure  the  ATE, which in these simulations, is equal to 5.
...
These different randomization strategies lead to different numbers of individuals assigned to treatment and control in each of the subgroups defined by values of `W` and `X`. To illustrate, consider a simulated experiment based on the complete assignment approach. We can tabulate the number of individuals assigned to treatment or control (`Z`) at different levels of `X`:
...
As you can  see, the two methods produce the same estimated ATE.
...
However, even  if we block, a regression estimator that uses covariates that were not part of the  blocked assignment can still yield improvements. For example, if  we block by `W`,  the regression estimator that adjusts for `X` still shows improvement over the difference-in-means estimator:
